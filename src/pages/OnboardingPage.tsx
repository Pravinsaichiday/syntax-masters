...
export default function OnboardingPage() {
  ...
  const handleFinish = () => {
    updateUser({ language, level, dailyTime, onboarded: true });
    navigate("/dashboard");
  };
  ...
  const steps = [
    ...
    },
    {
      ...
      subtitle: "So we can match your difficulty",
      options: LEVELS,
      value: level,
      onChange: setLevel,
    },
    {
      ...
      subtitle: "Consistency is key to improvement",
      options: TIMES,
      value: dailyTime,
          ...
          {steps.map((_, i) => (
            <div
              ...
              className={`h-1.5 w-12 rounded-full transition-colors ${
                i <= step ? "bg-gradient-gold" : "bg-surface-3"
              }`}
            />
          ))}
        </div>
