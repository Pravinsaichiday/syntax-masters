...
import { Flame, Target, TrendingUp, Zap, BookOpen, Trophy, Clock, ArrowRight } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const stats = [
    { icon: Target, label: "Problems Solved", value: user.solvedCount, color: "text-primary" },
    { icon: Zap, label: "XP Earned", value: user.xp.toLocaleString(), color: "text-primary" },
    { icon: Flame, label: "Day Streak", value: user.streak, color: "text-destructive" },
    { icon: TrendingUp, label: "Global Rank", value: `#${user.rank || "—"}`, color: "text-info" },
  ];
      ...
      <div className="container mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="mt-1 text-muted-foreground">
            {user.language && `${user.language} • `}{user.level && `${user.level} • `}{user.dailyTime && `${user.dailyTime}/day`}
          </p>
        </motion.div>
        ...
        <div className="mb-8">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/problems" className="text-sm text-primary hover:underline flex items-center gap-1">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Link to="/problems" className="flex items-center gap-3 rounded-xl border border-border bg-card p-5 transition-all hover:border-primary/30">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">Solve problems by topic</div>
            </div>
          </Link>
          <Link to="/contests" className="flex items-center gap-3 rounded-xl border border-border bg-card p-5 transition-all hover:border-primary/30">
            ...
            <div>
              <div className="font-semibold">Contests</div>
              <div className="text-sm text-muted-foreground">Compete live</div>
            </div>
          </Link>
          <Link to="/leaderboard" className="flex items-center gap-3 rounded-xl border border-border bg-card p-5 transition-all hover:border-primary/30">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">See global rankings</div>
            </div>
          </Link>
        </div>
      </div>
